...
## Grid & form - the cell layer

`many` and `one` are the data engine under firstly's **cell layer** - metadata-driven grids and
forms. firstly ships the headless primitives (`buildCells`, `<FF_Cell>`, the `FF_Config.cell` input
registry); the opinionated `FF_Grid` (CRUD grid) and `FF_Group` (bound-record form) are **boutique**
shells you copy from `src/boutique/grid` and own. Full chapter: [cell layer](/docs/svelte/cell).

```svelte
<FF_Grid entity={Task} selected={['title']} where={{ done: false }} strategy="listen" />
<FF_Group entity={Task} selected={['title']} mode="edit" />
```
