...
recomputed. Pass `{ refetch: true }` to re-pull authoritative items + aggregates from the server
after the local change:

```ts
await MyController.deleteWithSideEffects(id) // your server-side flow
if (ok) r.removeItem(id) // instant UX, client-only
// or, to also refresh totals/ordering from the server:
await r.removeItem(id, { refetch: true })
```
