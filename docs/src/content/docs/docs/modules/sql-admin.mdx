...
<SqlTokens capabilities={['read']} command={(t) => `FF_SQL_TOKEN=${t} pnpm sql:prod "select 1"`} />
```
...
- Revoke and delete are different gestures: a revoked token keeps its row and its calls (the audit
  trail), deleting one takes its calls with it. `SqlAdminController.purgeTokens()` deletes every
  expired or revoked token at once (the "Delete N dead" button).
...
firstly ships an `ff-sql` bin, so an app only needs a script that points it at an origin:

```json
// package.json
{ "scripts": { "sql:prod": "FF_SQL_ORIGIN=https://my.app node --env-file=.env node_modules/.bin/ff-sql" } }
```

```bash
FF_SQL_TOKEN=ffsql_… pnpm sql:prod "select count(*) from users"
pnpm sql:prod --json "select * from users limit 3"   # FF_SQL_TOKEN from .env
pnpm sql:prod <<'SQL'
