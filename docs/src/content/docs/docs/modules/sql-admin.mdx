...
```bash
FF_SQL_TOKEN=ffsql_… pnpm exec ff-sql --origin=https://my.app "select count(*) from users"
FF_SQL_TOKEN=ffsql_… pnpm exec ff-sql --origin=https://my.app --json "select * from users limit 3"
