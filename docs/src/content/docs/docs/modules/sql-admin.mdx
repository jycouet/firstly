...
sqlAdmin({
	// OPTIONAL - false removes the <SqlAdmin /> endpoint, keep only tokens.
	// console: true,
	tokens: {
...
- A token answers exactly one path (`/api/ff/sqlToken/sql`). Anywhere else it authenticates nobody.
...
```bash
curl -X POST https://my.app/api/ff/sqlToken/sql \
