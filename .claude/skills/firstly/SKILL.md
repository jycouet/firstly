...
From a terminal, use the `ff-sql` bin (`FF_SQL_TOKEN`, `FF_SQL_ORIGIN`, `--json`, stdin heredoc) rather than hand-rolling curl - and prefer a heredoc over nested shell quotes:
...
## `repoFetch` / `loadRepo` - reading in loads with API rules

`ff` is component-level (browser-only). In a SvelteKit **`load`**, a bare `repo()` reads the DB
privileged on SSR (API rules skipped - leaks) and calls the API on CSR: same line, different data,
plus a second query on hydration. Fix: bind a repo to `event.fetch`.
...
- `loadRepo` works for universal AND server loads (`satisfies PageLoad` or `PageServerLoad`); the
  event defaults to `LoadEvent | ServerLoadEvent`, so `{ params }`/`url` just work - annotate the
  event (`event: ServerLoadEvent`) for `cookies`/`locals`. In a server load, use it only when the
  load should see exactly what the API exposes (server fetch runs in-process, cookies forwarded);
  otherwise keep the privileged DB (gate rows with `backendPrefilter`). A bare `repo()` stays
  privileged either way; BackendMethods keep their own `allowed` gate.
