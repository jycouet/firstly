...
>() => {
  ...
  function passwordHash(password: string) {
    return bcrypt.hashSync(
      password,
      AUTH_OPTIONS.providers?.password?.algo?.bcrypt?.saltRounds ?? 10,
    )
  }
  ...
  return {
    ...
    strings: {
      ...
      cannotSignUp: AUTH_OPTIONS.strings?.cannotSignUp ?? "You can't signup by yourself! Contact the administrator.",
    }
  }
}
