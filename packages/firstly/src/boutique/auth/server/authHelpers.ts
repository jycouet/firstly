...
export const addRolesToUser = async (emails: string[], roles: string[]) => {
	...
	for (const nf of userNotFounds) {
		console.info(`addRolesToUser: User`, nf, 'not found, roles', roles)
	}
	...
	for (const user of users) {
		...
		if (rolesToAdd.length > 0) {
			...
			console.info(`addRolesToUser: User`, user.email, 'roles added', rolesToAdd)
		} else {
	...
	if (usersAlreadyHaveRoles.length > 0) {
		console.info(`addRolesToUser: Users`, usersAlreadyHaveRoles, 'already have roles', roles)
	}
}
