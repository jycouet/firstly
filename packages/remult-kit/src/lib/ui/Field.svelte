<script lang="ts" generics="T extends Record<any, any>">
	...
	const getLoadOptions = async (str: string) => {
		...
		const narrowFindWhere = cell.field?.options.narrowFind ? cell.field.options.narrowFind().where : {}
		...
		items = res.map((r) => getRepoDisplayValue('Field.svelte Select edit', metaTypeObj.repoTarget, r))
	}
